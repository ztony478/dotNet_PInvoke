...
namespace MarshalStruct_FieldMarshal
{
    class Program
    {
        ...
        private static void TestGetEmployeeEx()
        {
            ...
            Console.WriteLine("性别: {0}", employee.IsFamale ? "女" : "男");
            Console.WriteLine("工龄: {0}", employee.EmployedYear);
            Console.WriteLine("是否在总部: {0}", employee.IsInRedmond ? "是" : "否");
        }
        ...
        {
            Console.WriteLine("托管代码定义的结构体({0})在非托管代码中的大小为({1})字节", type.Name, Marshal.SizeOf(type));
        }
    }
}
